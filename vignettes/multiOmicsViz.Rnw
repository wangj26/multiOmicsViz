...
\section{Environment}
multiOmicsViz requires R version 3.3 or later, which can be downloaded from the website \href{http://www.r-project.org/}{http://www.r-project.org/}.  multiOmicsViz package requires the following packages: parallel ($>$3.3.2), doParallel ($>$1.0.10) and foreach ($>$1.4.0), which can be installed as follows.
...
outputfile <- paste(getwd(),"/heatmap",sep="")
...
This section describes the arguments of the multiOmicsViz function:
      ...
      1. \emph{sourceOmics}: A R matrix or data.frame object containing the omics data.
      ...
      4. \emph{targetOmicsList}: An R list object containing at most 3 target omics data. 
...
\subsection{Input}
	1. \emph{matrix1}: A R matrix or data.frame object containing the numeric values.
	
	2. \emph{matrix2}: A R matrix or data.frame object containing the numeric values.
	
	3. \emph{fdr}: The FDR threshold for identifying significant correlations.
